...
1. The owner selected `com.cheffodoggo.app`, and the coordinator created a draft Play app. Prepared values are in `production/`; recheck the current Console package/version and remaining store setup before uploading. The default QA identity is unchanged. Preserve the established signing identity and use a higher version code after any uploaded artifact.
...
3. A dedicated Cheffo upload key is now stored outside this repository at `%LOCALAPPDATA%\CheffoDoggo\Signing`, protected by current-user/SYSTEM ACL and a DPAPI CurrentUser password. Use `production/build-signed.ps1` as described in `production/README.md`; signing secrets enter Gradle only through temporary process environment. No key is copied from Recipe Reborn. Release candidate tasks require the verified package, `CHEFFO_RELEASE_READY=true`, and complete signing inputs. The flag approves a local build only, not device QA or store publication. Play App Signing setup and owner-managed portable encrypted backup remain separate gates; increase `CHEFFO_VERSION_CODE` above any uploaded artifact.
