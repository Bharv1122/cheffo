...
Run `node tests/verify-adult-confirmation.cjs` against the same local Vite server for the actual signup and AI confirmation UI. The fully synthetic fixture checks unchecked signup, user-metadata bypass prevention, existing-user acknowledgement, cancellation, server-save failure, happy path and account switching. It blocks every external request and creates no real accounts. The build also runs `scripts/verify-adult-confirmation.mjs`, which exercises the actual authenticated acknowledgement endpoint with mocked storage, including metadata preservation and rejected bypass attempts.
