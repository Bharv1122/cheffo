...
async function run(upstream,{token=true}={}){
...
async function failure(name,error,errorName='unknown',causeCode='unknown'){
 const {response,logs,state}=await run(()=>{throw error;});assert.equal(response.status,502,name);assert.deepEqual(await response.json(),{error:'Upstream LLM request failed'});
 assert.deepEqual(logs,[['[llm] upstream_failure',{stage:'fetch_exception',status:502,errorName,causeCode}]],name);checkSafe(logs);
