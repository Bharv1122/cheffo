...
import { formatVolumeAmountFromCups, recipeGramsPerCup } from '../../utils/calculator';
import { reviewRecipePlanning } from '../../utils/recipeInsights';
import type { DogProfile } from '../../types/dog';
import type { Recipe } from '../../types/recipe';

// Cups from the bowl's actual density, not the flat 240 g/cup baked into
// serving.cupsPerMeal. A vet reading this needs the per-meal volume to agree
// with the gram weight next to it.
function servingVolumePerMeal(recipe: Recipe): string {
  const cups = recipe.serving.gramsPerMeal / recipeGramsPerCup(recipe.ingredients);
  return formatVolumeAmountFromCups(cups);
}

function buildVetEmailTemplate(recipe: Recipe, dog: DogProfile | null): string {
  const planningReview = reviewRecipePlanning(recipe);
...
- Per meal: ${formatGrams(recipe.serving.gramsPerMeal)} / ${servingVolumePerMeal(recipe)}
...
Cheffo planning review:
- Status: ${planningReview.title}
- ${planningReview.summary}
${planningReview.issues.map(issue => `- ${issue}`).join('\n')}
        ...
        <div className="space-y-6">
          ...
          <section>
            ...
            <div className="text-sm space-y-1 text-[#1C1917]">
              ...
              <p><strong>Per meal:</strong> {formatGrams(recipe.serving.gramsPerMeal)} · {servingVolumePerMeal(recipe)}</p>
            ...
            <div className={[
              ...
              <p><strong>{planningReview.title}.</strong> {planningReview.summary}</p>
          ...
          <section className="border-t border-[#E7E5E4] pt-4">
            <p className="text-xs text-[#78716C] leading-relaxed italic">
              This document was generated by Cheffo Doggo (cheffodoggo.com), a homemade dog food planning tool. Cheffo Doggo provides general educational guidance and does not replace professional veterinary advice. All calorie estimates, portion sizes, and supplement ranges are educational starting points only. A licensed veterinarian or veterinary nutritionist should review all homemade diets before feeding, especially for puppies, seniors, pregnant/nursing dogs, and dogs with medical conditions.
            </p>
          </section>
        </div>
      </PageWrapper>
